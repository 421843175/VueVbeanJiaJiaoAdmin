  ...
  >
    <Form :model="record">
      ...
      <FormItem label="父级分类">
        <Row>
          <Col :span="12">
            <Select v-model:value="one" :options="gradeoptions">
              <!-- <SelectOption value="9">英语</SelectOption> -->
              <!-- 其他选项 -->
            </Select>
          </Col>
          <Col :span="12">
            <!-- <Select v-model:value="two" :options="options" v-show="record.level >= 2" -->
            <Select v-model:value="two" :options="options" /> </Col></Row
      ></FormItem>
      ...
      <FormItem label="排序">
        <InputNumber placeholder="倒序排序" v-model:value="record.sort" />
      </FormItem>
    </Form>
  </BasicModal>
</template>
