<template>
  ...
  >
    <Descriptions size="middle" :column="1">
      ...
      <Descriptions.Item label="投诉类型">{{ record.typeName }}</Descriptions.Item>
      <Descriptions.Item label="投诉内容">{{ record.content }}</Descriptions.Item>
      ...
      <Descriptions.Item label="端口">{{
        record.type == 1 ? '家长端' : '老师端'
      }}</Descriptions.Item>
      <Descriptions.Item label="创建时间">{{ formattedTime }}</Descriptions.Item>
      <Descriptions.Item label="是否回复">
        {{ record.isContact === 0 ? '未回复' : '已回复' }}
      </Descriptions.Item>
    </Descriptions>
  </BasicModal>
</template>
...
<script lang="ts" setup>
  ...
  const formattedTime = computed(() => {
    const date = new Date(record.value.createTime);
