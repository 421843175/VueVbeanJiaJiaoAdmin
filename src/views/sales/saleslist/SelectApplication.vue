<template>
  ...
  >
    <Descriptions size="middle" :column="1">
      ...
      <Descriptions.Item label="报名单号">{{ record.refundSn }}</Descriptions.Item>
      ...
      <Descriptions.Item label="报酬">{{ record.reward }}</Descriptions.Item>
      <Descriptions.Item label="申请退费金额">{{ record.messageFee }}</Descriptions.Item>
      <Descriptions.Item label="凭证">{{ record.proofText }}</Descriptions.Item>
      <Descriptions.Item label="凭证图片">
        <!-- <Avatar @click="createImgPreview({imageList:[globalURL+record.realAvatar]})" :src="(globalURL+record.realAvatar) || 'https://www.antdv.com/assets/logo.1ef800a8.svg'" />  -->
        <Image
          :src="globalURL + record.proofImages || 'https://www.antdv.com/assets/logo.1ef800a8.svg'"
          :height="50"
          :width="50"
        />
      </Descriptions.Item>
      <Descriptions.Item label="状态">
        {{ record.status === 0 ? '申请中' : record.status === 1 ? '已同意' : '已拒绝' }}
      </Descriptions.Item>

      <Descriptions.Item label="审核状态"> {{ record.status }}</Descriptions.Item>
    </Descriptions>
  </BasicModal>
</template>
...
<script lang="ts" setup>
  ...
  const formattedTime = computed(() => {
    const date = new Date(record.value.payEndTime);
