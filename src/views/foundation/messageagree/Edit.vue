  ...
  >
    <a-form :model="record">
      ...
      <a-form-item label="图文详情">
        <Tinymce ref="md" v-model="record.content" :subfield="false" defaultOpen="edit" />
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
    ...
    if (record.value.id != undefined) {
      try {
        const response = await axios.put(
          `${import.meta.env.VITE_GLOB_API_URL}${import.meta.env.VITE_GLOB_API_MY_URL_PREFIX}/admin/message`,
          record.value,
          {
            headers: {
              Authorization: useUserStore().getToken,
